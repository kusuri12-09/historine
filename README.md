...
기본 관리자 계정은 `.env` 값으로 설정한다.

`.env`를 만들지 않은 경우 개발 기본값은 다음과 같다.
...
관리자로 로그인하면 연표, 인물 백과, 사건 백과 데이터를 추가할 수 있다.

현재 등록 데이터는 서버 메모리에 저장되므로 개발 서버를 재시작하면 초기 샘플 데이터로 돌아간다.
...
관리자는 연표, 인물, 사건 데이터를 추가할 수 있다.

관리자 화면은 공개 메뉴에 노출하지 않고 `/admin` 경로로 접근한다.
